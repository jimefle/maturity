...
export default function Questionnaire({ subdomain, evaluationId, onFinishSubdomain }) {
  ...
  const handleChange = (value) => {
    setResponses((prev) => ({
      ...
      [currentQuestionId]: {
        value, 
        recommendationsIds: currentQuestion.options.find(opt => opt.value === value)?.recommendationsIds || []
      }
    }));
  };
  ...
  const handleNext = async () => {
    ...
    const nextId = currentQuestion.options.find(opt => opt.value === currentResponse.value)?.next;
    
    if (nextId) {
      setCurrentQuestionId(nextId);
    } else {
      const { level, prog } = await calculateSubdomainLevel(responses, questions);
  ...
  if (fError) return <p className="text-red-400 text-center">Error: {fError}</p>;
        ...
        <main className="max-w-xl h-fit bg-black/20 backdrop-blur-md border border-zinc-700 shadow-lg rounded-2xl p-8">
          ...
          <div key={currentQuestion.id} className="text-start">
            ...
            <div className="space-y-3">
              {currentQuestion.options.map((opt, index) => (
                <label key={index} className="flex items-center gap-2 cursor-pointer hover:text-purple-400 transition">
                  <input
                    type="radio"
                    name={`pregunta-${currentQuestionId}`}
                    value={opt.value}
                    checked={responses[currentQuestionId]?.value === opt.value}
            ...
            <div className="flex justify-end mt-6">
              <button
                disabled={!responses[currentQuestionId]?.value || sLoading}
                onClick={handleNext}
                className={`py-2.5 px-6 rounded-full font-semibold shadow-md transition-all duration-300 ${
                  !responses[currentQuestionId]?.value || sLoading
